...
// ðŸ‘‰ HELPER: Format dung lÆ°á»£ng file
...
// ðŸ‘‰ HELPER: Láº¥y icon theo Ä‘uÃ´i file
...
export default function ChatGroupBox({ selected, setUnreadMap, loadChats, onGroupDissolved }) {
  ...
  // ðŸ‘‰ Sá»¬A STATE: Chuyá»ƒn tá»« 'file' Ä‘Æ¡n láº» sang máº£ng 'files'
  ...
  const handleOpenForwardModal = async (msg) => {
    ...
    if (res?.success) {
      ...
      alert("KhÃ´ng thá»ƒ táº£i danh sÃ¡ch cuá»™c trÃ² chuyá»‡n Ä‘á»ƒ chuyá»ƒn tiáº¿p.");
    }
  };
  ...
  useEffect(() => {
    ...
    const handleReceive = (msg) => {
      ...
      // ðŸ‘‰ THÃŠM 2 DÃ’NG NÃ€Y (Äá»ƒ cháº·n tin nháº¯n do chÃ­nh mÃ¬nh gá»­i bá»‹ in ra 2 láº§n)
  ...
  // Helper Ä‘áº©y tá»«ng tin lÃªn giao diá»‡n & gá»i API
  ...
  const sendMessage = async () => {
    ...
    // Gá»­i FILE trÆ°á»›c (Láº·p máº£ng)
  ...
  // ================= FORMAT GIAO DIá»†N Tá»ªNG LOáº I TIN NHáº®N =================
  const renderMessage = (m, index) => {
    ...
    // Äá»•i background: Náº¿u lÃ  file thÃ¬ ná»n tráº¯ng cÃ³ viá»n má» cho Ä‘áº¹p, text thÃ¬ ná»n xanh/tráº¯ng
        ...
        <div className="shadow-sm position-relative" style={{ maxWidth: "70%", padding: m.type === "image" ? "4px" : "10px 14px", borderRadius: bubbleRadius, backgroundColor: highlightId === m._id ? "#ffe58f" : bubbleBg, color: "#000", border: m.type === "image" ? "none" : "1px solid #e1e4ea" }}>
          ...
          {m.isDeleted ? (
            ...
            <>
              ...
              {/* ðŸ‘‰ GIAO DIá»†N FILE DOC, PDF, XLS */}
              {m.type === "file" && (
    ...
    <div className="col-9 d-flex flex-column h-100 border-start border-end p-0 position-relative">
      <div className="p-3 border-bottom bg-white d-flex justify-content-between align-items-center shadow-sm">
        <div className="min-w-0">
          <h5 className="m-0 fw-bold text-truncate">{selected?.name || "Nhóm chat"}</h5>
        ...
        <div className="d-flex gap-2 align-items-center">
          <button
            ...
            style={{
              width: 40,
              height: 40,
              background: showSearchBox ? "#e5efff" : "#f1f3f5",
              color: showSearchBox ? "#0068ff" : "#495057",
            }}
            onClick={() => setShowSearchBox((prev) => !prev)}
            title="Tìm kiếm tin nhắn"
          >
            {showSearchBox ? <FaTimes size={15} /> : <FaSearch size={16} />}
          </button>
          <button className="btn btn-outline-primary" onClick={() => setShowGroupInfo(true)}>
            Thông tin nhóm
          </button>
        </div>
      </div>
      ...
      {groupDissolved && (
        <div className="px-3 py-2 bg-light border-top text-center text-muted" style={{ fontSize: 14 }}>
          Nhóm đã giải tán. Bạn chỉ có thể xem lại lịch sử trò chuyện.
        </div>
      )}
      ...
      {/* ðŸ‘‰ KHU Vá»°C HIá»‚N THá»Š DANH SÃCH FILE CHUáº¨N Bá»Š Gá»¬I */}
      {files.length > 0 && (
        // Sá»­a maxHeight tá»« '120px' thÃ nh '250px' vÃ  thÃªm padding 'p-3' cho thoÃ¡ng
      ...
      {groupDissolved ? (
        <div className="p-2 border-top bg-white text-center text-muted">
          Nhóm đã giải tán, không thể nhắn tin tiếp.
        </div>
      ) : (
