...
import { getConversations, getOrCreatePrivateConversationAPI } from "../api/chatApi";
...
export default function useChatPage() {
  ...
  const handleOpenFriendChat = async (friend) => {
    ...
    const res = await getOrCreatePrivateConversationAPI(friendId);
    if (!res?.success) {
      alert(res?.message || "Khong the mo cuoc tro chuyen");
      return;
    }

    const conversationId = res?.data?.conversationId || res?.data?._id || res?.conversationId;
    const conversationsRes = await getConversations();
    const conversationList = Array.isArray(conversationsRes?.data) ? conversationsRes.data : [];
    const normalizedList = conversationList.map(normalizeConversation).filter(Boolean);
    const conversation =
      normalizedList.find(
        (item) =>
          String(item?._id || item?.conversationId) === String(conversationId)
      ) ||
    ...
    setContacts((prev) => dedupeAndSortContacts([conversation, ...normalizedList, ...prev]));
    setSelected(conversation);
    setTab("chat");
    setUnreadMap((prev) => ({
      ...prev,
      [conversation._id || conversation.conversationId]: 0,
    }));
  };
